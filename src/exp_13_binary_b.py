...
# Binary classification: HC vs MG (EXCLUDING Probable MG)
BINARY_CLASS_DEFINITIONS = {
    'HC': {'path': 'Healthy control', 'label': 0},
    'MG': {'path': 'Definite MG', 'label': 1},
    # NOTE: Excluding 'Probable MG' to compare with experiment 13A
}
...
RESULTS_DIR = './results/exp_13_b'
EXP_PREFIX = 'EXP_13_BINARY_B_'
...
def prepare_binary_data(raw_items, f_out):
    """Prepare data for binary classification WITHOUT Probable MG."""
    f_out.write("="*80 + "\n")
    f_out.write("Phase: Binary Classification Data Preparation (WITHOUT Probable MG)\n")
    f_out.write("="*80 + "\n")
    print("\n" + "="*50 + "\nPreparing Binary Classification Data (WITHOUT Probable MG)...\n" + "="*50)
    ...
    for item in raw_items:
        if item['class_name'] == 'MG':
            # Only include Definite MG
            binary_items.append(item)
            class_counts['MG'] += 1
        elif item['class_name'] == 'HC':
            binary_items.append(item)
            class_counts['HC'] += 1
        elif item['class_name'] == 'Probable_MG':
            # Exclude Probable MG for this experiment
            excluded_count += 1
    
    f_out.write(f"Binary classification data prepared (WITHOUT Probable MG):\n")
    f_out.write(f"  HC (Healthy Control): {class_counts['HC']} samples\n")
    f_out.write(f"  MG (Definite MG only): {class_counts['MG']} samples\n")
    f_out.write(f"  Excluded (Probable MG): {excluded_count} samples\n")
    f_out.write(f"  Total included: {len(binary_items)} samples\n")
    f_out.write("-" * 80 + "\n\n")
    
    print(f"Binary data prepared: HC={class_counts['HC']}, MG={class_counts['MG']}, Excluded={excluded_count}")
    return binary_items
...
def run_statistical_models(master_df, numerical_features, results_dir, f_out):
    ...
    f_out.write("\n" + "="*80 + "\n")
    f_out.write("Phase: Statistical Models for Binary Classification (WITHOUT Probable MG)\n")
    f_out.write("="*80 + "\n")
    ...
    for model_name, model in models.items():
        accuracy, report, cm = train_and_evaluate_single_model(
            ...
            suffix="_Binary_B", random_state=RANDOM_STATE
        )
...
def run_deep_learning_models(binary_items, num_features, results_dir, f_out):
    ...
    f_out.write("\n" + "="*80 + "\n")
    f_out.write("Phase: Deep Learning Models (with 10x Subsampling, WITHOUT Probable MG)\n")
    f_out.write("="*80 + "\n")
...
def main():
    ...
    print("="*80)
    print("Starting Experiment 13B: Binary Classification (HC vs MG)")
    print("EXCLUDING 'Probable MG' from MG class (Definite MG only)")
    print("="*80)
    ...
    with open(summary_filepath, 'w', encoding='utf-8') as f_report:
        f_report.write("="*80 + "\n")
        f_report.write("Experiment 13B: Binary Classification (HC vs MG) - WITHOUT Probable MG\n")
        f_report.write("="*80 + "\n")
        ...
        f_report.write(f"NOTE: This experiment EXCLUDES 'Probable MG' data\n")
        ...
        # Load all data first (including Probable MG) then filter it out
        all_class_definitions = {
            'HC': {'path': 'Healthy control', 'label': 0},
            'MG': {'path': 'Definite MG', 'label': 1},
            'Probable_MG': {'path': 'Probable MG', 'label': 1},  # Load but will exclude
        }
        
        raw_items_list = load_raw_sequences_and_labels(
            BASE_DIR, all_class_definitions, FEATURE_COLUMNS, 
        ...
        # 2. Prepare Binary Classification Data (excluding Probable MG)
        ...
        # 6. Final Summary
        f_report.write("\n" + "="*80 + "\n")
        f_report.write("EXPERIMENT 13B FINAL RESULTS SUMMARY (WITHOUT Probable MG)\n")
        f_report.write("="*80 + "\n")
        ...
        # Comparison note
        f_report.write("\n" + "="*80 + "\n")
        f_report.write("COMPARISON WITH EXPERIMENT 13A:\n")
        f_report.write("="*80 + "\n")
        f_report.write("Experiment 13A (WITH Probable MG):\n")
        f_report.write("  - Statistical Models: LDA (68.97%), Logistic Regression (66.95%), Linear SVM (66.04%)\n")
        f_report.write("  - Deep Learning Models: Small BiGRU+Attention (72.36%), Simple LSTM (64.90%), Simple GRU (64.16%)\n")
        f_report.write("  - Data: 1,331 sequences (510 HC, 821 MG including Probable MG)\n")
        f_report.write("\nExperiment 13B (WITHOUT Probable MG):\n")
        f_report.write("  - Results shown above\n")
        f_report.write("  - This allows comparison to justify inclusion/exclusion of Probable MG data\n")
        
        f_report.write("\n" + "="*80 + "\n")
        f_report.write("End of Experiment 13B Report\n")
        f_report.write("="*80 + "\n")
    
    print(f"\nExperiment 13B completed successfully!")
