"""
Multi-Head Trajectory Prototype Network Experiment

Tests the hybrid architecture combining:
- MultiHeadProtoNet's structural constraint (2 prototypes per head)
- PPTMasked's temporal dynamics (trajectory prototypes with velocities)

PRIMARY SUCCESS CRITERIA (LOCKED - DO NOT MODIFY):
1. Accuracy >= 70.9% (must match or exceed MultiHeadProtoNet)
2. Temporal Pattern: Late > Early discrimination in >= 2/3 folds

If accuracy < 70.9%, the experiment FAILS regardless of temporal pattern.
"""
...
# PRIMARY SUCCESS CRITERIA - LOCKED
PRIMARY_ACCURACY_THRESHOLD = 0.709  # Must match MultiHeadProtoNet
PRIMARY_TEMPORAL_FOLDS_THRESHOLD = 2  # At least 2/3 folds must pass


@dataclass
class MHTConfig:
    """Configuration for MultiHeadTrajectoryProtoNet experiment."""
...
class MHTTrainer:
    """Trainer for MultiHeadTrajectoryProtoNet."""
...
def compute_temporal_metrics(
    ...
    """Compute temporal discrimination metrics on validation set."""
    model.eval()
...
def run_experiment(config: MHTConfig, output_dir: str, verbose: bool = True):
    """
    Run the MultiHeadTrajectoryProtoNet experiment with 5-fold cross-validation.
    """
    ...
    if verbose:
        ...
        print("\n" + "="*70)
        print("MULTI-HEAD TRAJECTORY PROTOTYPE NETWORK EXPERIMENT")
        print("="*70)
        print(f"\nPRIMARY SUCCESS CRITERIA (LOCKED):")
        print(f"  1. Accuracy >= {PRIMARY_ACCURACY_THRESHOLD:.1%}")
        print(f"  2. Temporal Pattern: Late > Early in >= {PRIMARY_TEMPORAL_FOLDS_THRESHOLD}/{config.n_folds} folds")
        print(f"\nConfiguration:")
    ...
    for fold, (train_idx, val_idx) in enumerate(cv.split(X, y, patient_ids)):
        ...
        # Compute temporal discrimination
        temporal_metrics = compute_temporal_metrics(model, val_loader, device)

        if verbose:
            print(f"\nTemporal Discrimination:")
            print(f"  Early: {temporal_metrics['early_discrimination']:.4f}")
            print(f"  Late: {temporal_metrics['late_discrimination']:.4f}")
            print(f"  Late - Early: {temporal_metrics['late_minus_early']:.4f}")
            print(f"  Pattern Pass: {'YES' if temporal_metrics['temporal_pattern_pass'] else 'NO'}")
    ...
    accuracy_pass = mean_accuracy >= PRIMARY_ACCURACY_THRESHOLD
    temporal_pass = n_temporal_pass >= PRIMARY_TEMPORAL_FOLDS_THRESHOLD

    summary = {
        'config': asdict(config),
        'primary_criteria': {
            'accuracy_threshold': PRIMARY_ACCURACY_THRESHOLD,
            'temporal_folds_threshold': PRIMARY_TEMPORAL_FOLDS_THRESHOLD,
        },
        'results': {
            ...
            'both_pass': accuracy_pass and temporal_pass,
        },
        'fold_results': fold_results,
        'timestamp': datetime.now().isoformat(),
    }

    # Print summary with explicit pass/fail per MANDATORY_GUIDELINES
    if verbose:
        print("\n" + "="*70)
        print("EXPERIMENT RESULTS")
        print("="*70)

        print(f"\n  PRIMARY CRITERION 1: ACCURACY")
        print(f"  {'='*50}")
        print(f"  Result: {mean_accuracy:.1%} +/- {std_accuracy:.1%}")
        print(f"  Threshold: >= {PRIMARY_ACCURACY_THRESHOLD:.1%}")
        print(f"  Status: {'PASS' if accuracy_pass else '** FAIL **'}")

        print(f"\n  PRIMARY CRITERION 2: TEMPORAL PATTERN")
        print(f"  {'='*50}")
        print(f"  Folds with Late > Early: {n_temporal_pass}/{config.n_folds}")
        print(f"  Threshold: >= {PRIMARY_TEMPORAL_FOLDS_THRESHOLD}/{config.n_folds}")
        print(f"  Status: {'PASS' if temporal_pass else '** FAIL **'}")

        print(f"\n  {'='*50}")
        if accuracy_pass and temporal_pass:
            print(f"  OVERALL: SUCCESS - Both primary criteria met")
        elif not accuracy_pass:
            print(f"  OVERALL: ** FAILURE ** - Accuracy criterion not met")
            print(f"  Note: Temporal pattern is irrelevant when accuracy fails")
        else:
            print(f"  OVERALL: ** FAILURE ** - Temporal pattern criterion not met")

        print(f"\n  Per-fold breakdown:")
        for r in fold_results:
            temporal_status = "PASS" if r['temporal']['temporal_pattern_pass'] else "FAIL"
            print(f"    Fold {r['fold']}: acc={r['accuracy']:.1%}, "
                  f"late-early={r['temporal']['late_minus_early']:+.4f} ({temporal_status})")
...
if __name__ == '__main__':
    ...
    output_dir = f"results/ccece/multi_head_trajectory/{timestamp}"
