...
# --- Basic Setup ---
BASE_DIR = './data'
EXP_NAME = 'EXP_2' # Reverted experiment name
...
TARGET_SEQ_LEN_PERCENTILE = 95 # MEMORY FIX: Reduced percentile from 98 to 95

# --- Model & Training Hyperparameters (Revised for better performance & memory efficiency) ---
N_FOLDS = 5
EPOCHS = 100 # Increased epochs, but with early stopping
BATCH_SIZE = 32 # Reduced batch size to conserve memory
LEARNING_RATE = 0.0005
WEIGHT_DECAY = 1e-2 # Adjusted for AdamW
HIDDEN_DIM = 128 # MEMORY FIX: Reverted to 128 to avoid memory issues
N_LAYERS = 2
DROPOUT_PROB = 0.5
EARLY_STOPPING_PATIENCE = 15 # Slightly more patience
LABEL_SMOOTHING = 0.1 # PERFORMANCE: New regularization technique
...
# --- Data Normalization and Early Stopping Classes ---
...
# --- PERFORMANCE: Function to calculate class weights ---
...
class SaccadeRNN_V2(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers, seq_len, dropout_prob):
        ...
        self.fc = nn.Sequential(
            nn.Linear(hidden_dim * 2, 128), nn.ReLU(), nn.Dropout(dropout_prob),
...
def plot_confusion_matrix(cm, class_names, title, filepath):
    # Use 'g' format for heatmap to handle both int and float
...
if __name__ == '__main__':
    ...
    log_message(f_log, f"Experiment: {EXP_NAME} - Advanced RNN Classification (AdamW, Label Smoothing)")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y, groups)):
        ...
        # PERFORMANCE: Apply class weights AND label smoothing
        criterion = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=LABEL_SMOOTHING)
        
        # PERFORMANCE: Use AdamW optimizer
        optimizer = torch.optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
        
        # BUG FIX: Remove verbose=True
